...
public class Region : IEnumerable<Tile>
{
    ...
    public bool RemoveMember(Tile tile)
    {
        ...
        if (!RegionManager.IsRegionContiguous(newRegion))
            return false;
    ...
    {
        List<List<Tile>> islands = RegionManager.IdentifyIslands(members);
    ...
    private static void Trace(List<Tile> region, bool outwardEdge, out List<Vector3> edgeInside, out List<Vector3> edgeOutside, List<Vector3> vertices, List<Vector2> uv, List<int> triangles)
    {
        ...
        void NextEdge()
        {
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                int insideLength = Map.MeshRes;

                Vector3 outsidePoint = map.OnTerrain(t.transform.position + new Vector3(cosAngles[e], 0, sinAngles[e]) * size);
                if (!firstEdge)
                {
                    ...
                    for (int j = 1; j < res; j++)
                    {
                        Vector3 intermediatePoint = map.OnTerrain(Vector3.Lerp(new Vector3(previousOutsidePoint.x, 0, previousOutsidePoint.z), new Vector3(outsidePoint.x, 0, outsidePoint.z), (float)j / res));
                ...
                if (n != null && n.inFloodFillSet)
                {
                    if (outwardEdge)
                    {
                        insidePoint = map.OnTerrain(t.transform.position + Vector3.LerpUnclamped(new Vector3(cosAngles[e], 0, sinAngles[e]), new Vector3(cosAngles[(e + 1) % 6], 0, sinAngles[(e + 1) % 6]), -1f / res) * size);
                    ...
                    {
                        insidePoint = map.OnTerrain(t.transform.position + Vector3.Lerp(new Vector3(cosAngles[e], 0, sinAngles[e]), new Vector3(cosAngles[(e + 1) % 6], 0, sinAngles[(e + 1) % 6]), 1f / res) * size);
                ...
                if (outwardEdge)
                {
                    insidePoint = map.OnTerrain(t.transform.position + new Vector3(cosAngles[e], 0, sinAngles[e]) * size * (1 + 1f / res));
