...
Shader "LowPoly/SimpleWater"
{
	Properties
	{
		...
		_WavesAmplitude("WavesAmplitude", Vector) = (1,1,1,0)
	...
	SubShader
	{
		...
		uniform float4 _WavesAmplitude;

		void vertexDataFunc( inout appdata_full v )
		{
			float3 ase_vertex3Pos = v.vertex.xyz;
			float3 ase_vertexNormal = v.normal.xyz;
			v.vertex.xyz += ( ( sin( ( ( _WavesAmount * ase_vertex3Pos.z ) + _Time.y ) ) * ase_vertexNormal ) * _WavesAmplitude.xyz );
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float2 panner22 = ( uv_WaterNormal + 1.0 * _Time.y * float2( -0.003,0 ));
			float2 panner19 = ( uv_WaterNormal + 1.0 * _Time.y * float2( 0.004,0.004 ));
			float3 normal = BlendNormals( UnpackScaleNormal( tex2D( _WaterNormal, panner22 ) ,_NormalScale ) , UnpackScaleNormal( tex2D( _WaterNormal, panner19 ) ,_NormalScale ) );
			o.Normal = normal;
			...
			float4 screenColor65 = tex2D(_GrabTexture, ase_screenPos164.xy + normal * _Distortion);

			float4 screenColor65_nodistort = tex2D(_GrabTexture, ase_screenPos.xy);
			half refrFix = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, ase_screenPos164.xy + normal * _Distortion);

			float eyeDepth1 = LinearEyeDepth(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(ase_screenPos))));
			float eyeDepthDistorted = LinearEyeDepth(UNITY_SAMPLE_DEPTH(tex2D(_CameraDepthTexture, ase_screenPos164.xy + normal * _Distortion)));
			
			if (eyeDepth1)
			{
				//screenColor65 = screenColor65_nodistort;
			}

			float temp_output_89_0 = abs((eyeDepth1 - ase_screenPos.w));
			float temp_output_89_1 = abs((eyeDepthDistorted - ase_screenPos.w));

			float depth = saturate(pow((temp_output_89_0 + _WaterDepth), _WaterFalloff));
			float depthDistorted = saturate(pow((temp_output_89_1 + _WaterDepth), _WaterFalloff));

			float4 result = lerp(_DeepColor, _ShalowColor, depthDistorted);
			float foam = (saturate(pow((temp_output_89_0 + _FoamDepth), _FoamFalloff)) * tex2D(_Foam, panner116).r);

			//half4 distortOffset = half4(temp_output_24_0.xz * _Distortion, 0, 0);
			//half4 screenWithOffset = i.screenPos + distortOffset;
			//half4 grabWithOffset = grabPassPos + distortOffset;
			//
			//half4 rtRefractionsNoDistort = tex2Dproj(_GrabTexture, UNITY_PROJ_COORD(grabPassPos));
			//half refrFix = SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(grabWithOffset));
			//half4 rtRefractions = tex2Dproj(_GrabTexture, UNITY_PROJ_COORD(grabWithOffset));
			//
			//if (LinearEyeDepth(refrFix) < i.screenPos.w)
			//	rtRefractions = rtRefractionsNoDistort;

			result = lerp(result, screenColor65, depthDistorted);
			result = lerp(result, _FoamColor, foam);
			o.Albedo = result.rgb;
			float specular = lerp( _WaterSpecular , _FoamSpecular , foam);

			o.Specular = specular;
			float smoothness = lerp(_WaterSmoothness, _FoamSmoothness , foam);
