Shader "Custom/Tree"
{
	Properties
	{
		...
		_RimPower("Rim Power", Range(0.25, 8.0)) = 3
	...
	SubShader
	{
		...
		Pass
		{
			...
			vertexOutput vert(appdata v)
			{
				...
				float3 modelPos = mul(unity_ObjectToWorld, v.vertex).xyz;
				float2 uv = modelPos.xz * _WindDistortionMap_ST.xy + _WindDistortionMap_ST.zw + _WindFrequency * _Time.y;
				float2 windSample = (tex2Dlod(_WindDistortionMap, float4(uv, 0, 0)).xy * 2 - 1) * _WindStrength;

				float h = saturate(v.vertex.y / _Height);
				h *= h;

				float2 offset = windSample.xy * asin(h);
				offset = mul(unity_WorldToObject, float3(offset.x, 0, offset.y)).xz;
			...
			half4 frag(vertexOutput input) : SV_Target
			{
				...
				float rim = 1.0 - saturate(dot(input.normal, input.viewDir));

				col.rgb *= max((diffuse + specular) * atten * _LightColor0, UNITY_LIGHTMODEL_AMBIENT * _Ambient);

				col.rgb += _RimColor.rgb * _LightColor0.rgb * _RimColor.a * pow(rim, _RimPower);
